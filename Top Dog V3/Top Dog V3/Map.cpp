...
{
	string sInputName = "Tiles/", sPicName = "Tiles/", sTemp = "";
	int iTemp = 0;
	fstream fMap;
	sInputName += sMapName + ".txt";
	sPicName += sMapName + ".png";
	SetMap(sPicName);
	fMap.open(sInputName.c_str());
	if (fMap.good())
	{
		SetDimensions(iWidth, iHeight);
		int iLoopNum = (GetWidth() / Tile::GetSize()) * (GetHeight() / Tile::GetSize());
...
void Map::AddTile(int r, int c, int x, int y, int pp, vector<Object*>& vObj)
{
	...
	pos.push_back(m_vTiles.size() - 1);
	m_vvTilePos.push_back(pos);
...
Tile* Map::FindEmptyPickupPoint(int iPickupType, int iPickupNum)
{
	int iTilesFound = 0;
	for (m_vTilesIT = m_vTiles.begin(); m_vTilesIT != m_vTiles.end(); m_vTilesIT++)
	{
		...
		{
			if ((*m_vTilesIT)->GetHasPickup() == false && (iTilesFound == iPickupNum || iPickupNum == -1))
			{
			...
			if ((*m_vTilesIT)->GetHasPickup() == false && (iTilesFound < iPickupNum))
			{
				iTilesFound++;
			}
		}
	...
	ResetPickupPoint(iPickupType);
	
	return NULL;
}

void Map::ResetPickupPoint(int iPickupType)
{
	for (m_vTilesIT = m_vTiles.begin(); m_vTilesIT != m_vTiles.end(); m_vTilesIT++)
	{
		if ((*m_vTilesIT)->GetID() == iPickupType)
		{
			if ((*m_vTilesIT)->GetHasPickup() == true)
			{
				(*m_vTilesIT)->SetHasPickup(false);
			}
		}
	}
}

string Map::GetWayStarData()
{
	string returnString = "";
	for (m_vTilesIT = m_vTiles.begin(); m_vTilesIT != m_vTiles.end(); m_vTilesIT++)
	{
		returnString += (*m_vTilesIT)->GetWalkable();
		returnString += ",";
	}
	return returnString;
}
