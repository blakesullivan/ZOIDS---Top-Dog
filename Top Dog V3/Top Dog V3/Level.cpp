...
void Level::Draw(SDL_Surface* screen, float fDT)
{
	...
	{
		if ((m_vpObj[i]->GetState() == stateAlive || (m_vpObj[i]->GetState() == stateRespawn && m_vpObj[i]->GetType() == typeCharacter)) && m_vpObj[i]->GetType() != typeTile)
		{
			if (m_vpObj[i]->GetType() == typeCharacter)
			{
				if (dynamic_cast<Character*>(m_vpObj[i])->GetCharID() == cidRedler && m_iMyID != cidRedler && dynamic_cast<Character*>(m_vpObj[i])->GetIsUsingEnergy() == true)
				{//do nothing
				}
				else
...
void Level::Update(float fDeltaTime)
{
	...
	for (int i = m_vpObj.size() - 1; i >= 0; i--)
	{
		...
		if (m_vpObj[i]->GetState() == stateDead && m_vpObj[i]->GetType() == typeCharacter)
		{
			...
			SetSpawnPosition(dynamic_cast<Character*>(m_vpObj[i])->GetCharID());
		}
		else if (m_vpObj[i]->GetState() == stateDead && m_vpObj[i]->GetType() == typePickup)
		{
			cout << "testing for dead pickup\n";
		}
...
void Level::AddBullet(int cID, bool bUseAlt)
{
	...
	//cout << "Adding Bullet for: " << cID << " BulletsLeftBefore: " << c->GetStats()->GetBulletsLeft();
	//if (cID == cidBuster) {system("PAUSE");}
	...
	//cout << " BulletsLeftAfter: " << c->GetStats()->GetBulletsLeft() << endl;
	//if (cID == cidRedler) {system("PAUSE");}
}
...
void Level::AddExplosion(Object* obj)
{
	...
	else if (obj->GetType() == typeCharacter)
	{
		...
		m_pSM->PlaySoundEffect(soundDead, soundDead-1);
	}
		...
		if ((*m_vpObjIT)->GetID() != myID && (*m_vpObjIT)->GetState() == stateAlive)
		{
			if ((*m_vpObjIT)->GetType() == searchType || searchType == -1)
			{
				...
				if (bSearchFound == true)
				{
					...
					{
						if (searchType == typeCharacter)
						{
							if (dynamic_cast<Character*>((*m_vpObjIT))->GetCharID() == cidRedler)
							{
								if (dynamic_cast<Character*>((*m_vpObjIT))->GetIsUsingEnergy() == false)
								{
									temp = (*m_vpObjIT);
...
void Level::CheckForCollisions()
{
	if (m_bIsHost == true)
	{
		...
		for (m_vpObjIT = m_vpObj.begin(); m_vpObjIT != m_vpObj.end(); m_vpObjIT++)
		{
			if ((*m_vpObjIT)->GetType() == typeCharacter)
			{
				if (dynamic_cast<Character*>((*m_vpObjIT))->GetCharID() == m_iMyID || dynamic_cast<Character*>((*m_vpObjIT))->GetIsBot() == true)
				{
					if ((*m_vpObjIT)->GetState() == stateAlive)
					{
						//CheckCollisionAgainst((*m_vpObjIT));
						Tile* currTile = m_pMap->GetTile(dynamic_cast<Character*>((*m_vpObjIT))->GetX(), dynamic_cast<Character*>((*m_vpObjIT))->GetY());
						CheckCollisionAgainstPos(currTile->GetCol(), currTile->GetRow());
						//break;
					}
				}
			}
			else if ((*m_vpObjIT)->GetType() == typeBullet)
			{
				if (dynamic_cast<Bullet*>((*m_vpObjIT))->GetBullType() == btMissile)
				{
					if ((*m_vpObjIT)->GetState() == stateAlive)
					{
						//CheckCollisionAgainst((*m_vpObjIT));
						Tile* currTile = m_pMap->GetTile(dynamic_cast<Bullet*>((*m_vpObjIT))->GetX(), dynamic_cast<Bullet*>((*m_vpObjIT))->GetY());
						CheckCollisionAgainstPos(currTile->GetCol(), currTile->GetRow());
						//break;
					}
				}
...
void Level::CheckCollisionAgainstPos(int x, int y)
{
...
void Level::UpdateCharacter(int cid, float x, float y, float dir, bool shot, bool energyStart, float health, float bl, float el)
{
	if (m_bIsHost == true && cid == -1) {return;}

	//cout << "Updating cid: " << cid << endl;
	...
	if (shot == true) {AddBullet(cid, false);}

	if (energyStart == true && temp->GetIsUsingEnergy() == false)
		{StartEnergy(cid);}
	else if (energyStart == false && temp->GetIsUsingEnergy() == true)
		{StopEnergy(cid);}

	temp->GetStats()->SetHealth(health);
	temp->GetStats()->SetBulletsLeft(bl);
	temp->GetStats()->SetEnergy(el);
}

void Level::DrawZabatRadar(SDL_Surface* screen, float fDT)
{
...
void Level::SetSpawnPosition(int cID)
{
	float fTempDeg = 0;
	Character* temp = GetCharacter(cID);
	fTempDeg = atan( (1024 - temp->GetX()) / (768 - temp->GetY()) ) * 180 / M_PI;
	if (768 > temp->GetY()) {fTempDeg += 180;}

	temp->GetStats()->SetDirection(fTempDeg);
}