...
void CharacterSelect::Init()
{
	...
	m_Characters = NULL;
	m_Characters = IMG_Load("Images/Characters.png");
...
void CharacterSelect::ManageButtonPress()
{
	...
	if (m_bIsRefreshing == false)
	{
		...
		if (iButtonPressed == buttonCharBE) 
		{ConnectCharacter(buttonCharBE);}
		if (iButtonPressed == buttonCharPT) 
		{ConnectCharacter(buttonCharPT);}
		if (iButtonPressed == buttonCharRN) 
		{ConnectCharacter(buttonCharRN);}
		if (iButtonPressed == buttonCharRD) 
		{ConnectCharacter(buttonCharRD);}
		if (iButtonPressed == buttonCharSS) 
		{ConnectCharacter(buttonCharSS);}
		if (iButtonPressed == buttonCharZB) 
		{ConnectCharacter(buttonCharZB);}
		/*if (iButtonPressed >= buttonCharBE && iButtonPressed <= buttonCharZB)
		{
			m_pNC->SetCharacter(0, iButtonPressed - 20, "AI TEST#");
			m_pNC->SetCharSelected(iButtonPressed - 20);
			m_pBM->FindButton(scenePlay)->SetClickable(true);
			m_pBM->FindButton(scenePlay)->SetOutlined(true);
		}*/
	}
...
void CharacterSelect::SendForAllCharacters()
{
	basePacket* cData = new basePacket();

	cData->id = CLIENT_ALL_CHARACTER;

	m_pNC->Send(cData, sizeof(*cData));

	delete[] cData;

	SDL_Delay(1000);
	while (m_pNC->Receive() == true)
	{
		cout << "Got Packet." << endl;
		CharacterSelected();
	}
...
void CharacterSelect::DrawCharacters(SDL_Surface* screen)
{
	...
	iTextPosX[0][1] = 160; iTextPosY[0][1] = 180; sText[0][1] = "Health: 150";
	iTextPosX[0][2] = 160; iTextPosY[0][2] = 210; sText[0][2] = "Speed/Agility: 1/2";
	...
	iTextPosX[1][2] = 130; iTextPosY[1][2] = 420; sText[1][2] = "Speed/Agility: 3/4";
	iTextPosX[1][3] = 100; iTextPosY[1][3] = 450; sText[1][3] = "Weapon: Machine Gun";
	iTextPosX[1][4] = 100; iTextPosY[1][4] = 480; sText[1][4] = "Special: Missile";
	...
	iTextPosX[2][2] = 140; iTextPosY[2][2] = 630; sText[2][2] = "Speed/Agility: 4/3";
	...
	iTextPosX[3][2] = 660; iTextPosY[3][2] = 210; sText[3][2] = "Speed/Agility: 3/3";
	...
	iTextPosX[4][2] = 660; iTextPosY[4][2] = 420; sText[4][2] = "Speed/Agility: 5/2";
	...
	iTextPosX[5][2] = 660; iTextPosY[5][2] = 630; sText[5][2] = "Speed/Agility: 2/1";
