...
void Join::AddButtons()
{
	...
	//CHANGE BACK TO COMMENTED WHEN READY TO NETWORK AGAIN
	m_pBM->AddButton(514, 730, "Join", 100, 30, false, true, cN, cH, cD, false, sceneCharacter);
	//m_pBM->AddButton(514, 730, "Join", 100, 30, true, true, cN, cH, cD, false, sceneCharacter);
...
void Join::ConnectToServer(int iSvrNum)
{
	...
	else
	{
		...
		m_pBM->FindButton(m_aSvrButtons[iSvrNum].buttID)->SetClickable(false);
		m_pBM->FindButton(m_aSvrButtons[iSvrNum].buttID)->SetOutlined(false);

		m_pBM->FindButton(buttonName)->SetClickable(false);
		m_pBM->FindButton(buttonName)->SetOutlined(false);
	}
}
