...
void Play::Update(SDL_Surface* screen, float fDeltaTime)
{
	...
	//do something
	if (m_pLevel->GetCharacter(m_iMyID)->GetState() != stateDead)
	{
		bool bMoved = false;
		...
		static float siPacketTimer = 0; static int siPacketCount = 0;
		siPacketTimer += fDeltaTime;
		if (bMoved == true || (bOverEnergy == true || bEnergyStarted == true) || m_pLevel->GetCharacter(m_iMyID)->GetFired() == true)
		{
			if ((siPacketCount < 40 && siPacketTimer < 1) || (bOverEnergy == true || bEnergyStarted == true) || m_pLevel->GetCharacter(m_iMyID)->GetFired() == true)
			{
				SendCharacterUpdate(m_pLevel->GetCharacter(m_iMyID), bOverEnergy);
				siPacketCount++;
			}
		}
		if (m_pNC->GetIsHost() == true && (siPacketCount < 40 && siPacketTimer < 1))
		{
			for (int i = 0; i < 6; i++)
			{
				if (m_pLevel->GetCharacter(i)->GetIsBot() == true)
				{
					SendCharacterUpdate(m_pLevel->GetCharacter(i));
					siPacketCount++;
				}
			}
		}
...
void Play::SetupLevel()
{
	...
	m_HUD.Init(m_pLevel->GetCharacter(m_iMyID), m_pMT);
	m_pSM->PlayMusic(m_pNC->GetMapID(), 100);
}
...
void Play::ResponseToReceive()
{
	...
	if (m_pNC->GetPacketReceived()->id == PLAYER_UPDATE)
	{
		...
		if (mData->userID != m_pNC->GetServerID())
		{
			m_pLevel->UpdateCharacter(mData->charID, mData->x, mData->y, mData->dir, mData->shot, mData->energyStart, mData->health, mData->bullLeft, mData->engLeft);
		}
	}
...
void Play::SendCharacterUpdate(Character* cData, bool bOverrideEnergy)
{
	movePacket* mData = new movePacket();
	//Character* cData = m_pLevel->GetCharacter(m_iMyID);

	mData->id = PLAYER_UPDATE;
	mData->userID = m_pNC->GetServerID();
	mData->charID = cData->GetCharID();
	mData->x = cData->GetX();
	mData->y = cData->GetY();
	mData->dir = cData->GetStats()->GetDirection();
	mData->shot = cData->GetFired();
	if (bOverrideEnergy == true)
		{mData->energyStart = true;}
	else
		{mData->energyStart = cData->GetIsUsingEnergy();}
	mData->health = cData->GetStats()->GetHealth();
	mData->bullLeft = cData->GetStats()->GetBulletsLeft();
	mData->engLeft = cData->GetStats()->GetEnergy();
